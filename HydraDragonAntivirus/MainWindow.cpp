// TODO: Auto Remove or Continue customazation can be added here or ask user etc. also you need do this for normal malware scanner after you stop malware or not via clamav remove yara remove but not for ransom remove, Optimization no multiple detections add kill mechanism before move quarantine  add full scan quick scan add clamav with kill then quarantime remove form list after quarantine or delete file how many detected add new change detector auto update definations when program start etc. 
...
static const uint32 kMsgRemoveAll = 'dall'; // Message for deleting all threats
              ...
              B_ASYNCHRONOUS_CONTROLS | B_QUIT_ON_WINDOW_CLOSE)
{
    ...
    // Create an item with the default message and add it to the list
    BStringItem* item = new BStringItem("Scan:\n");
    fFileListView->AddItem(item); // Add the item to the list view

    fFileListView->SetFontSize(12); // Set font size for better readability
